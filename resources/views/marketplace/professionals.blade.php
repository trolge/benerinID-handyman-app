...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-emergency { background-color: var(--primary); color: white; padding: 0.6rem 1rem; border-radius: 8px; font-weight: 600; font-size: 0.875rem; transition: background 0.2s; cursor: pointer;}
        ...
        .dropdown-item { padding: 0.75rem 1rem; color: var(--text-dark); font-size: 0.875rem; font-weight: 500; border-radius: 8px; transition: background 0.15s; display: flex; align-items: center; gap: 0.75rem; cursor:pointer;}
        .dropdown-item:hover { background: var(--bg-page); }
        .dropdown-item.text-danger { color: #dc2626; }
        .dropdown-item.text-danger:hover { background: #fef2f2; }

        .container { max-width: 800px; margin: 0 auto; padding: 2rem 5%; }

        .page-header { margin-bottom: 2rem; }
        .page-title { font-size: 2rem; font-weight: 800; letter-spacing: -0.5px; margin-bottom: 0.5rem; }
        .page-subtitle { color: var(--text-muted); font-size: 1rem; }

        .search-bar { display: flex; align-items: center; background: white; border: 1px solid var(--border-color); border-radius: 12px; padding: 0.25rem 1rem; margin-bottom: 1.5rem; box-shadow: 0 1px 2px rgba(0,0,0,0.05);}
        .search-bar svg { color: var(--text-muted); margin-right: 0.5rem; }
        .search-bar input { border: none; outline: none; padding: 0.75rem 0.5rem; width: 100%; font-size: 0.95rem; font-family: inherit;}

        .pills-container { display: flex; gap: 0.5rem; overflow-x: auto; padding-bottom: 1rem; margin-bottom: 1rem; scrollbar-width: none; }
        .pills-container::-webkit-scrollbar { display: none; }
        
        .pill { padding: 0.5rem 1.25rem; border-radius: 9999px; font-size: 0.875rem; font-weight: 600; white-space: nowrap; transition: all 0.2s; border: 1px solid transparent; background: #e2e8f0; color: var(--text-dark); }
        .pill:hover { background: #cbd5e1; }
        .pill.active { background: var(--primary); color: white; box-shadow: 0 2px 4px rgba(26,86,219,0.2); }

        .pro-card { background: var(--card-bg); border-radius: 16px; padding: 1.5rem; border: 1px solid var(--border-color); margin-bottom: 1.5rem; transition: transform 0.2s, box-shadow 0.2s; position: relative;}
        .pro-card:hover { transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); }
        
        .pro-header { display: flex; gap: 1rem; align-items: flex-start; margin-bottom: 1rem; }
        .pro-avatar { width: 72px; height: 72px; border-radius: 12px; background: #374151; overflow: hidden; position: relative; flex-shrink: 0;}
        .pro-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .verified-badge { position: absolute; bottom: -6px; right: -6px; background: var(--primary); color: white; width: 22px; height: 22px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid white;}
        
        .pro-info { flex: 1; }
        .pro-badge { text-transform: uppercase; font-size: 0.7rem; font-weight: 700; color: var(--primary); letter-spacing: 0.5px; margin-bottom: 0.25rem; display: block;}
        .pro-name { font-size: 1.25rem; font-weight: 800; color: var(--text-dark); margin-bottom: 0.25rem; display: flex; align-items: center; justify-content: space-between;}
        
        .rating-chip { display: inline-flex; align-items: center; gap: 0.25rem; background: var(--bg-page); padding: 0.2rem 0.5rem; border-radius: 6px; font-size: 0.8rem; font-weight: 700; color: var(--text-dark); }
        .rating-chip svg { color: var(--accent); fill: var(--accent); }

        .pro-desc { font-size: 0.9rem; color: var(--text-muted); margin-bottom: 1.25rem; line-height: 1.6; }

        .pro-tags { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-bottom: 1.5rem; }
        .tag { background: #dbeafe; color: #1e40af; padding: 0.25rem 0.75rem; border-radius: 6px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;}

        .pro-actions { display: flex; gap: 1rem; align-items: center; justify-content: space-between;}
        .btn-text { color: var(--primary); font-weight: 700; font-size: 0.875rem; display: flex; align-items: center; gap: 0.25rem; cursor: pointer; }
        .btn-solid { background: var(--primary); color: white; padding: 0.75rem 1.5rem; border-radius: 8px; font-weight: 600; font-size: 0.95rem; text-align: center; transition: background 0.2s; flex-shrink: 0; box-shadow: 0 4px 6px -1px rgba(26,86,219,0.2);}
...
<body>
    ...
    <div class="container">
        ...
        <div class="pills-container">
            <a href="{{ route('professionals.index') }}" class="pill {{ $currentCategory == 'all' ? 'active' : '' }}">All Experts</a>
            @foreach($categories as $cat)
                <a href="{{ route('professionals.index', ['category' => $cat['slug']]) }}" class="pill {{ $currentCategory == $cat['slug'] ? 'active' : '' }}">{{ $cat['name'] }}</a>
            @endforeach
        </div>
        ...
        <div class="pros-list">
            @if($handymen->isEmpty())
                <div class="empty-state">
                    <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"></circle><line x1="8" y1="12" x2="16" y2="12"></line></svg>
                    ...
                    <a href="{{ route('professionals.index') }}" style="color: var(--primary); font-weight: 600; display: inline-block; margin-top: 1rem;">Clear Filters</a>
                </div>
            @endif

            @foreach($handymen as $pro)
                <div class="pro-card" data-name="{{ strtolower($pro->name) }}" data-category="{{ strtolower($pro->primaryCategory) }}">
                    <div class="pro-header">
                        <div class="pro-avatar">
                            @if($pro->avatar)
                                <img src="{{ asset('storage/' . $pro->avatar) }}" alt="{{ $pro->name }}">
                            @else
                                <div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:white; font-size:1.5rem; font-weight:bold;">{{ substr($pro->name, 0, 1) }}</div>
                            @endif
                            <!-- Verified Badge -->
                    ...
                    <p class="pro-desc">
                        @if($pro->Expertise)
                            {{ $pro->Expertise }}
                        @else
                            Professional specialist delivering top-tier services in {{ $pro->primaryCategory }}. Highly rated for prompt response and excellent craftsmanship. Available for urgent bookings.
                        @endif
                    </p>
